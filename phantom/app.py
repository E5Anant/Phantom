...
class Codesmith:
    def __init__(self, llm_instance: GroqLLM | Openai | Gemini | Anthropic):
        """
        Initialize Codesmith with a pre-configured LLM instance.
        """
        self.system_prompt = COMBINED_SYSTEM_PROMPT
        llm = type(llm_instance)
            
            # Extract current LLM parameters
        llm_params = {}
        if hasattr(llm_instance, 'api_key'):
            llm_params['api_key'] = llm_instance.api_key
        if hasattr(llm_instance, 'model'):
            llm_params['model'] = llm_instance.model
        if hasattr(llm_instance, 'max_tokens'):
            llm_params['max_tokens'] = llm_instance.max_tokens
        if hasattr(llm_instance, 'temperature'):
            llm_params['temperature'] = llm_instance.temperature
